...
def symbol_adjacent(schematic, symbols, part_num, location:Point):
    ...
    points_to_check = []
    if prev_row >= 0 and prev_col >= 0 and next_row < len(schematic) and next_col < len(schematic[0]):  # this part is not on an edge
    ...
    for y, row in enumerate(schematic):
        ...
        for x, char in enumerate(row):
            if char.isdigit():  # we found a part number
                ...
                part_num += char
                # print(part_num)
                if x+1 >= len(row) or not row[x+1].isdigit(): # we are done parsing the part number
...
def is_gear(schematic, parts, gear_loc):
    ...
    if prev_row >= 0 and prev_col >= 0 and next_row < len(schematic) and next_col < len(schematic[0]):  # this gear is not on an edge
